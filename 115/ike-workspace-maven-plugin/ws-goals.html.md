...
The `ws:*` plugin goals are a typed console over a **working set of one or more** git repositories. With a `workspace.yaml` manifest they coordinate cross-repository operations — fanning out across every checked-out subproject in topological order so feature branches stay aligned, parent versions cascade in lockstep, and releases hold their own tag history even when seven repositories ship together.

Run from a single repository with no `workspace.yaml`, the working-tree and lifecycle goals — `ws:commit`, `ws:push`, `ws:pull`, `ws:sync`, `ws:scaffold`, `ws:release`, `ws:feature-****`**, `ws:sibling-create` — operate on that one repo: a *working set of one** (ike-issues#601 / #611 / #703). Goals that act on the cross-repository graph (`ws:align`, `ws:graph`, `ws:overview`, `ws:verify-convergence`, …) still require a workspace. Run `ws:help` to see, per goal, which run on a single repo and which need a workspace.
