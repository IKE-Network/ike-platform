...
After a new ike-platform release, update the workspace to use the new parent version:

```
mvn ws:set-parent-draft -DnewVersion=22    # preview changes
mvn ws:set-parent-publish -DnewVersion=22  # apply to all POMs
```

This updates the root POM and all component POMs in one pass. If you omit `-DnewVersion`, the goal prompts interactively.
