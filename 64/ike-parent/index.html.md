...
- `network.ike.tooling:ike-maven-plugin` at `180` — `ike:*` goals (`release-publish`, `prepare-release`, `inject-breadcrumb`, etc.).
- `network.ike.docs:ike-doc-maven-plugin` at `37` — `idoc:*` render goals (AsciiDoc, multi-renderer PDF wrappers, etc.).
