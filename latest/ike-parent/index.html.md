...
- `network.ike.tooling:ike-maven-plugin` at `256` — `ike:*` goals (`release-draft`, `release-publish`, `inject-breadcrumb`, etc.).
- `network.ike.docs:ike-doc-maven-plugin` at `111` — `idoc:*` render goals (AsciiDoc, multi-renderer PDF wrappers, etc.).
