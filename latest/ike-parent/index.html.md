...
- `network.ike.tooling:ike-maven-plugin` at `254` — `ike:*` goals (`release-draft`, `release-publish`, `inject-breadcrumb`, etc.).
- `network.ike.docs:ike-doc-maven-plugin` at `109` — `idoc:*` render goals (AsciiDoc, multi-renderer PDF wrappers, etc.).
